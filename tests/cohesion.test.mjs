...
import { SURFACE_COLUMNS, SURFACE_ROWS, WORLD_WIDTH } from '../src/constants.ts'

// Measure visible detached pools, rather than requiring particles to be frozen.
function detachedFraction(fluid, left, right) {
  const minX = Math.ceil((left/WORLD_WIDTH+.5)*SURFACE_COLUMNS)
  const maxX = Math.floor((right/WORLD_WIDTH+.5)*SURFACE_COLUMNS)-1
  ...
  for (let y = 0; y < SURFACE_ROWS; y++) {
    for (let x = minX; x <= maxX; x++) {
      const start = y*SURFACE_COLUMNS+x
      if (seen[start] || fluid.height[start] < .0025) continue
      let head = 0, tail = 1, mass = 0
      queue[0] = start; seen[start] = 1
      while (head < tail) {
        const i = queue[head++], px = i%SURFACE_COLUMNS, py = Math.floor(i/SURFACE_COLUMNS)
        mass += fluid.height[i]
        for (const j of [px > minX ? i-1 : -1, px < maxX ? i+1 : -1, py > 0 ? i-SURFACE_COLUMNS : -1, py < SURFACE_ROWS-1 ? i+SURFACE_COLUMNS : -1]) {
