...
import { Fluid } from '../src/simulation.ts'
import { COLON_PARTICLE_START, DIGIT_PARTICLE_COUNT, PARTICLE_COUNT, PARTICLE_DIAMETER } from '../src/constants.ts'
...
test('stray fluid at a separator and between colon coils is recaptured by live magnets', () => {
  ...
  const probes = [DIGIT_PARTICLE_COUNT,DIGIT_PARTICLE_COUNT+1,DIGIT_PARTICLE_COUNT+2,DIGIT_PARTICLE_COUNT+3,COLON_PARTICLE_START,COLON_PARTICLE_START+1,COLON_PARTICLE_START+2,COLON_PARTICLE_START+3]
  for (let k = 0; k < 4; k++) {
    fluid.x[DIGIT_PARTICLE_COUNT+k] = -1.56
    fluid.y[DIGIT_PARTICLE_COUNT+k] = .1+k*.05
    fluid.x[COLON_PARTICLE_START+k] = .25
    fluid.y[COLON_PARTICLE_START+k] = -.075+k*.05
  }
  ...
  for (const i of probes) {
    const digit = i < COLON_PARTICLE_START ? 1 : -1
...
test('mass remains finite, nonnegative and conserved through changing fields and release', () => {
  ...
  assert.equal(fluid.x.length, PARTICLE_COUNT)
  assert.ok(Math.abs(fluid.initialMass - 1568) < 1e-10, 'finer resolution must retain the original liquid charge')
  assert.ok(Math.abs(DIGIT_PARTICLE_COUNT * PARTICLE_DIAMETER ** 2 - 340 * .048 ** 2) <= PARTICLE_DIAMETER ** 2 / 2 + 1e-10, 'the packing area of each numeral must be preserved within particle-count rounding')
  for (let frame = 0; frame < 600; frame++) {
    ...
    if (frame % 30 === 0) {
      ...
      for (const h of fluid.height) {
        assert.ok(Number.isFinite(h) && h >= 0)
      }
      for (const y of fluid.y) {
        assert.ok(Number.isFinite(y) && y >= -1.37 && y <= 1.37)
      }
    }
  }
})
